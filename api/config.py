    ...
    if "providers" in generator_config:
        for provider_id, provider_config in generator_config["providers"].items():
            ...
            if provider_config.get("client_class") in CLIENT_CLASSES:
                ...
                # For OpenAIClient, check if custom environment variable names are specified
                if provider_config["client_class"] == "OpenAIClient":
                    env_api_key_name = provider_config.get("env_api_key_name", "OPENAI_API_KEY")
                    env_base_url_name = provider_config.get("env_base_url_name", "OPENAI_BASE_URL")
                    
                    # Store these in provider config for later use
                    provider_config["client_init_kwargs"] = {
                        "env_api_key_name": env_api_key_name,
                        "env_base_url_name": env_base_url_name
                    }
                    
                    # Log if custom environment variables are being used
                    if env_api_key_name != "OPENAI_API_KEY" or env_base_url_name != "OPENAI_BASE_URL":
                        logger.info(f"Provider '{provider_id}' will use custom env vars: {env_api_key_name}, {env_base_url_name}")
...
def load_embedder_config():
    ...
    for key in ["embedder", "embedder_ollama", "embedder_google", "embedder_custom_openai"]:
        if key in embedder_config and "client_class" in embedder_config[key]:
            class_name = embedder_config[key]["client_class"]
            if class_name in CLIENT_CLASSES:
                embedder_config[key]["model_client"] = CLIENT_CLASSES[class_name]
                
                # For OpenAIClient in embedder_custom_openai, store custom env var names
                if key == "embedder_custom_openai" and class_name == "OpenAIClient":
                    env_api_key_name = embedder_config[key].get("env_api_key_name", "OPENAI_API_KEY")
                    env_base_url_name = embedder_config[key].get("env_base_url_name", "OPENAI_BASE_URL")
                    
                    embedder_config[key]["client_init_kwargs"] = {
                        "env_api_key_name": env_api_key_name,
                        "env_base_url_name": env_base_url_name
                    }
                    
                    logger.info(f"Custom embedder will use env vars: {env_api_key_name}, {env_base_url_name}")
...
def get_embedder_config():
    ...
    elif embedder_type == 'custom_openai' and 'embedder_custom_openai' in configs:
        return configs.get("embedder_custom_openai", {})
    else:
...
def get_embedder_type():
    ...
    Returns:
        str: 'ollama', 'google', 'custom_openai', or 'openai' (default)
    """
    if EMBEDDER_TYPE == 'custom_openai':
...
if embedder_config:
    for key in ["embedder", "embedder_ollama", "embedder_google", "embedder_custom_openai", "retriever", "text_splitter"]:
...
def get_model_config(provider="google", model=None):
    ...
    Parameters:
        provider (str): Model provider ('google', 'openai', 'openrouter', 'ollama', 'bedrock', 'custom_openai')
    ...
    # Add client initialization kwargs if present (for custom OpenAI clients)
    if "client_init_kwargs" in provider_config:
        result["client_init_kwargs"] = provider_config["client_init_kwargs"]
