...
class OpenAIClient(ModelClient):
    ...
    def init_sync_client(self):
        ...
        if not api_key:
            raise ValueError(
                f"{self._env_api_key_name} environment variable is not set. Please configure your OpenAI API key."
            )
    ...
    def call(self, api_kwargs: Dict = {}, model_type: ModelType = ModelType.UNDEFINED):
        ...
        if model_type == ModelType.EMBEDDER:
            try:
                print(f"DEBUG: Calling embedding API with kwargs: {api_kwargs}")
                response = self.sync_client.embeddings.create(**api_kwargs)
                print(f"DEBUG: Embedding response type: {type(response)}")
                if hasattr(response, 'data'):
                    print(f"DEBUG: Embedding response data length: {len(response.data)}")
                    if len(response.data) > 0:
                        print(f"DEBUG: First embedding length: {len(response.data[0].embedding)}")
                else:
                    print("DEBUG: Embedding response has no data attribute")
                return response
            except Exception as e:
                print(f"DEBUG: Error calling embedding API: {e}")
                raise e
        elif model_type == ModelType.LLM:
    ...
    async def acall(
        ...
        if model_type == ModelType.EMBEDDER:
            try:
                print(f"DEBUG: Calling async embedding API with kwargs: {api_kwargs}")
                response = await self.async_client.embeddings.create(**api_kwargs)
                print(f"DEBUG: Async embedding response type: {type(response)}")
                return response
            except Exception as e:
                print(f"DEBUG: Error calling async embedding API: {e}")
                raise e
        elif model_type == ModelType.LLM:
