...
from api.config import get_model_config, configs, OPENROUTER_API_KEY, OPENAI_API_KEY, replace_env_placeholders
...
class ChatCompletionRequest(BaseModel):
    ...
    type: Optional[str] = Field("github", description="Type of repository (e.g., 'github', 'gitlab', 'gitcode', 'bitbucket')")
...
- Use markdown formatting to improve readability
        ...
        if request.provider == "ollama":
            ...
            client_init_kwargs = full_model_config.get("client_init_kwargs", {})

            # Check if an API key is set
            env_api_key_name = client_init_kwargs.get("env_api_key_name", "OPENAI_API_KEY")
            api_key_set = os.getenv(env_api_key_name)
            if not api_key_set:
                logger.warning(f"{env_api_key_name} not configured, but continuing with request")

            # Initialize OpenAI client with custom env vars if specified
            model = OpenAIClient(**client_init_kwargs)
            model_kwargs = {
                "model": request.model,
                "stream": True,
                "temperature": model_config["temperature"]
        ...
        try:
            if request.provider == "ollama":
                ...
                try:
                    ...
                    logger.error(f"Error with OpenAI-compatible API: {str(e_openai)}")
                    # Get the correct env var name for error message
                    full_model_config = get_model_config(request.provider, request.model)
                    env_api_key_name = full_model_config.get("client_init_kwargs", {}).get("env_api_key_name", "OPENAI_API_KEY")
                    error_msg = f"\nError with OpenAI-compatible API: {str(e_openai)}\n\nPlease check that you have set the {env_api_key_name} environment variable with a valid API key."
                    await websocket.send_text(error_msg)
                    # Close the WebSocket connection after sending the error message
                    await websocket.close()
            elif request.provider == "azure":
                ...
                try:
                    ...
                    if request.provider == "ollama":
                        ...
                        try:
                            ...
                            logger.error(f"Error with OpenAI-compatible API fallback: {str(e_fallback)}")
                            # Get the correct env var name for error message
                            full_model_config = get_model_config(request.provider, request.model)
                            env_api_key_name = full_model_config.get("client_init_kwargs", {}).get("env_api_key_name", "OPENAI_API_KEY")
                            error_msg = f"\nError with OpenAI-compatible API fallback: {str(e_fallback)}\n\nPlease check that you have set the {env_api_key_name} environment variable with a valid API key."
                            await websocket.send_text(error_msg)
                    elif request.provider == "azure":
                        try:
                            # Create new api_kwargs with the simplified prompt
