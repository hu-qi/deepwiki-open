...
def download_repo(repo_url: str, local_path: str, repo_type: str = None, access_token: str = None) -> str:
    """
    Downloads a Git repository (GitHub, GitLab, GitCode, or Bitbucket) to a specified local path.
    ...
    try:
        ...
        if access_token:
            ...
            elif repo_type == "gitlab" or repo_type == "gitcode":
...
def prepare_data_pipeline(
    ...
    Args:
        ...
        is_ollama_embedder (bool, optional): DEPRECATED. Use embedder_type instead.
                                           If None, will be determined from configuration.
        embedding_api_key (str, optional): Override API key for OpenAI-compatible embedders.
        embedding_base_url (str, optional): Override base URL for OpenAI-compatible embedders.
        embedding_model (str, optional): Override embedding model name.
    ...
    embedder = get_embedder(
        ...
        override_api_key=embedding_api_key,
        override_base_url=embedding_base_url,
        override_model=embedding_model
    )
...
def get_gitcode_file_content(repo_url: str, file_path: str, access_token: str = None) -> str:
    """
    Retrieves the content of a file from a GitCode repository using the GitCode API v5.

    Args:
        repo_url (str): The GitCode repository URL (e.g., "https://gitcode.com/username/repo")
        file_path (str): The path to the file within the repository (e.g., "src/main.py")
    ...
    Raises:
        ValueError: If the file cannot be fetched or if the URL is not a valid GitCode URL
    """
    try:
        ...
        # Check if it's a GitCode-like URL structure
        path_parts = parsed_url.path.strip('/').split('/')
        if len(path_parts) < 2:
        ...
        # Use GitCode API v5 to get file content
        # The API endpoint for getting file content is: /api/v5/repos/{owner}/{repo}/contents/{path}
        api_url = f"{api_base}/api/v5/repos/{owner}/{repo}/contents/{file_path}"
        ...
        try:
            ...
            raise ValueError("Invalid response from GitCode API")

        # Check if we got an error response
        if "message" in content_data and "documentation_url" in content_data:
            raise ValueError(f"GitCode API error: {content_data['message']}")

        # GitCode API v5 returns file content as base64 encoded string (like GitHub)
        if "content" in content_data and "encoding" in content_data:
            if content_data["encoding"] == "base64":
                # The content might be split into lines, so join them first
                content_base64 = content_data["content"].replace("\n", "")
...
def get_file_content(repo_url: str, file_path: str, repo_type: str = None, access_token: str = None) -> str:
    """
    Retrieves the content of a file from a Git repository (GitHub, GitLab, GitCode, or Bitbucket).
    ...
    if repo_type == "github":
        ...
        raise ValueError("Unsupported repository type. Only GitHub, GitLab, GitCode, and Bitbucket are supported.")
...
class DatabaseManager:
    ...
    def prepare_database(self, repo_url_or_path: str, repo_type: str = None, access_token: str = None,
                         ...
                         embedding_model: str = None) -> List[Document]:
        """
    ...
    def _extract_repo_name_from_url(self, repo_url_or_path: str, repo_type: str) -> str:
        ...
        if repo_type in ["github", "gitlab", "bitbucket"] and len(url_parts) >= 5:
            ...
            # GitCode URL format: https://gitcode.net/owner/repo
